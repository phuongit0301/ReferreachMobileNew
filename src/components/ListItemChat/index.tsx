...
const ListItemChat: React.FC<Props> = ({
  ...
  if (items?.attributes?.related_chat_contexts?.length > 0) {
    return items?.attributes?.related_chat_contexts.map((item: any, index: number) => {
      ...
      //   if (item?.chat_box_type === CHAT_BOX_TYPE_ENUM.INTRODUCED && item?.current_user_role !== 'introducee') {
      //     return (
      //       <TouchableOpacity
      ...
      //   if (
      //     (+dataUser?.id === +userState?.userInfo?.id && item?.current_user_role === 'asker') ||
      //     item?.current_user_role === 'introducee'
      //   ) {
      //     return (
      //       <TouchableOpacity
      //         key={`chat-item-${item?.id}-${index}`}
      ...
      //                     style={
      //                       introducee
      //                         ? introducer?.avatar_metadata?.avatar_url
      //                           ? styles.iconProtect1
      //                           : styles.iconProtect3
      //                         : styles.iconProtect1
      //                     }
      ...
      //               <View style={[GlobalStyles.flexColumn, styles.contentContainer]}>
      //                 <View style={[GlobalStyles.flexRow, GlobalStyles.pr30, GlobalStyles.mb3, styles.headerContainer]}>
      //                   <Paragraph bold600 h6 title={`${introducer?.full_name} `} />
